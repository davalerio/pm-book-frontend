...
export class Form1Component implements OnInit {
  ...
  valProfile() {
    ...
    if (profile === 1) {
      ...
      this.selectForm.get('businessname')?.setValue('PERUMONEY');
    } else {
      this.showData = true;
  ...
  AllAgency() {
    this.apiService.getAllAgency().subscribe((data) => {
      this.agencies = data;
    });
  }
  ...
  SendEmail(form: any) {
    ...
    let params = {
      ...
      day: day,
      hour: hour,
      agency_id: this.selectForm.value.agency,
    ...
    if (this.selectForm.invalid) {
      ...
      this.apiService.postSendEmailPerumoney(params).subscribe((data) => {
